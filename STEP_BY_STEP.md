...
| `Node.js 20 detected without native WebSocket support` | Supabase Realtime en Node 20 sin paquete `ws` / transport | Deploy con `ws` + `realtime.transport` en `src/lib/supabase.ts` (o `RAILPACK_NODE_VERSION=22` en Railway) |
| `ZodError` / `Invalid url` en `MCP_PUBLIC_URL` | URL con barra final (`...railway.app/`) u espacios; Zod valida antes de normalizar | El servidor recorta `/` al final; en Railway podés dejar la URL con o sin `/` |
